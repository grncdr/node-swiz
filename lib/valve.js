    ...
    func: function(value, baton, callback) {
      ...
      async.map(value,
                function(item, itercb) {
                  checkChain(item, chain, self.baton, itercb);
                },
                callback);
    },
...
Chain.prototype.isHash = function(keyChain, valueChain) {
  ...
  var iter = function(memo, item, callback) {
    ...
    checkChain(key, keyChain, self.baton, function(err, cleanedKey) {
        if (err) {
          callback('Key ' + key + ': ' + err);
        } else {
          checkChain(value, valueChain, self.baton, function(err, cleanedValue) {
            if (err) {
