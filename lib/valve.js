...
Chain.prototype.getValidatorPos = function(name) {
  // TODO: Add _validatorCount private attribute. Some validators are added to
  // the begging of the array so this method is not ideal.
  var i, len, validator;
  ...
  for (i = 0, leni = def.length; i < leni; i++) {
    ...
    for (j = 0, lenj = fields.length; j < lenj; j++) {
      ...
      if (field.ignorePublic === true) {
        ...
        validity[group][name] = new Chain().enumerated(field.enumerated);
      }
      else {
      ...
      if (field.src) {
        validity[group][name].rename(field.src).optional();
      }
    }
  }
