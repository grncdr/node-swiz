...
Swiz.prototype.serializeJson = function(obj) {
  return JSON.stringify(obj, this._options.stripNulls ? pruneNull : null, 4);
};
...
Swiz.prototype._serializeXml = function(stype, elem, obj, key) {
  elem = elem || null;
  var i, def, fields, field, keys, item, type, src, value, result, selem, matches, attribute;
  ...
  if (obj instanceof Array) {
    ...
    for (i = 0; i < keys.length; i++) {
      if (obj.hasOwnProperty(keys[i])) {
        ...
        if (this._options.stripNulls && (obj[keys[i]] === null || obj[keys[i]] === undefined)) {
          continue;
        }
        else {
