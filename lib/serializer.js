...
Swiz.prototype._startTag = function(name, result) {
  if (name) {
    result.push('<', name, '>');
  }
};
...
Swiz.prototype._endTag = function(name, result) {
  if (name) {
    result.push('</', name, '>');
  }
};
...
Swiz.prototype._serializeXml = function(obj, key, result) {
  var i, def, stype, item, src;

  // Treat each member of an array as a separate property with the same key
  if (obj instanceof Array) {
    for (i = 0; i < obj.length; i++) {
      this._serializeXml(obj[i], key, result);
    }
  }
  ...
  // Look up object definitions, serialize each defined property
    ...
    this._startTag(key, result);
    this._startTag(stype, result);

    for (i = 0; i < def.length; i++) {
      item = def[i];
      src = item[0];
      this._serializeXml(obj[src], this.xmlEscapeString(src), result);
    }

    this._endTag(stype, result);
    this._endTag(key, result);
  }

  // Serialize individual values
  else {
    this._startTag(key, result);
    result.push(this.xmlEscapeString(obj.toString()))
    this._endTag(key, result);
  }
};
...
Swiz.prototype.serializeXml = function(obj) {
  ...
  if (obj instanceof Array) {
    this._startTag('group', result);
    this._serializeXml(obj, null, result);
    this._endTag('group', result);
  } else {
    this._serializeXml(obj, null, result);
  }
