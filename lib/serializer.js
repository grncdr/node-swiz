...
Swiz.prototype.serializeXml = function(obj) {
  ...
  elem = this._serializeXml(null, elem, obj, null);
 ...
 */
Swiz.prototype._serializeXml = function(stype, elem, obj, key) {
  elem = elem || null;
  var i, def, keys, item, type, src, value, result, selem, matches, attribute;

  if (obj instanceof Array) {
    if (stype && obj.length > 0) {
      def = this.getDefinitionForKey(stype, key);
      type = def[1].type;
      // TODO: refactor into a separate function
      matches = type.match('(list|array)<(.*?)>');
    ...
    for (i = 0; i < obj.length; i++) {
      this._serializeXml(stype, selem, obj[i], key);
    }
    ...
    for (i = 0; i < def.length; i++) {
      item = def[i];
      src = item[0];
      this._serializeXml(stype, selem, obj[src], src);
    }
    ...
    for (i = 0; i < keys.length; i++) {
      if (obj.hasOwnProperty(keys[i])) {
        ...
        if (this.options.stripNulls && (obj[keys[i]] === null || obj[keys[i]] === undefined)) {
          ...
          this._serializeXml(null, elem, obj[keys[i]], src);
        }
      }
    }
    ...
    value = obj.toString();
    def = (stype) ? this.getDefinitionForKey(stype, key) : null;
    attribute = (def) ? def[1].attribute : false;
 ...
 */
Swiz.prototype.getDefinitionForKey = function(stype, key) {
  var defs = this.defs[stype], i, len, def;
